...
export function Keystroke(handler, priority?): void {
...
const getNextWordOffset = (text, offset, isBackward) => {
    let previousType
    while ((isBackward && offset > 0) || (!isBackward && offset < text.length)) {
        const currentType = charTypes[isBackward ? text[offset - 1] : text[offset]] || 'word';
...
export const defaultKeystrokes = [
    ...
    new Keystroke(function (evt, state, editor) {
        ...
        if (!state.selection) {
            var isJump = (isMac && evt.altKey) || (!isMac && evt.ctrlKey)
            if (isJump) {
